...
import livehq.{Command, Internal, Log, StandardPcDetails}
...
object Subscriber {
  ...
  case class Offer(identifier: String, offer: SessionDescription) extends Command
...
class Subscriber(webRtcHelper: WebRtcHelper, subscriberCallback: SubscriberCallback) extends PersistentActor with ActorLogging {
  ...
  override def receiveCommand: Receive = {
    ...
    //      publisherRegion ! RequestPeerConnection("1", "1")
